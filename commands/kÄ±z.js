...
data: new SlashCommandBuilder()
  .setName("kız")
  .setDescription("Kişiyi kız olarak kaydedersiniz")
...
mentioned.roles.add(interaction.guild.roles.cache.get(db.fetch(`sistem_`+ interaction.guild.id).kız)).catch(err => {
  interaction.reply({ embeds: [Embed("Hata tespit edildi.", "Kullanıcıya rol verilemedi.", "error")]})
  console.error(err)
})
...
interaction.reply({ embeds: [Embed("Bir kişi kız olarak kaydedildi.", "Bilgiler verilmiştir.", "info").addField(`Kayıt edilen kişi`, `${mentioned}`).addField(`Kayıt edilen kişinin adı`, `${isim}`).addField(`Kayıt edilen kişinin yaşı`, `${yaş}`)]})

let log = client.channels.cache.get(db.fetch("sistem_" + interaction.guild.id).kanal2)
let embed = Embed("Yeni bir kişi kayıt oldu.","Aramıza hoşgeldin <@" + mentioned.user.id + ">","info").addField("Cinsiyet:","Kız")
